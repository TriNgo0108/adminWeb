<?php 
    ...
    $curl= curl_init("localhost/adminWE/apis/getSearchQuality.php?name=".$_POST['name']);
    curl_setopt($curl,CURLOPT_RETURNTRANSFER,1);
    $quality = json_decode(curl_exec($curl));
    curl_close($curl);
    $numPage = $numPage ?? 0;
    $pagination = computePagination($quality->quality,$numPage);
    $curl = curl_init("localhost/adminWE/apis/searchProduct.php?name=$_POST[name]&start=$pagination[pageStart]&limit=8");
    curl_setopt($curl,CURLOPT_RETURNTRANSFER,1);
